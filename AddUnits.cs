                                ...
                                using (SqlCommand findUnitCmd = new SqlCommand(findUnitQuery, conn, transaction))
                                {
                                    ...
                                    if (existingUnit != null && existingUnit != DBNull.Value)
                                    {
                                        ...
                                        string checkIfOwnedQuery = @"
                                            SELECT r.FirstName, r.LastName, r.HomeownerID
                                        ...
                                        using (SqlCommand cmd = new SqlCommand(checkIfOwnedQuery, conn, transaction))
                                        {
                                            ...
                                            using (SqlDataReader rdr = cmd.ExecuteReader())
                                            {
                                                if (rdr.Read())
                                                {
                                                    int existingHomeownerId = Convert.ToInt32(rdr["HomeownerID"]);
                                                    string existingOwnerName = $"{rdr["FirstName"]} {rdr["LastName"]}";

                                                    if (existingHomeownerId != _homeownerId)
                                                    {
                                                        rdr.Close();
                                                        MessageBox.Show(
                                                            $"This unit is already actively owned by {existingOwnerName} (Homeowner ID: {existingHomeownerId}).\n\n" +
                                                            "You cannot register another owner for this unit.",
                                                            "Unit Already Owned",
