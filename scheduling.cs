...
namespace RECOMANAGESYS
{
    public partial class scheduling : UserControl
    {
        ...
        private void StyleDataGridView()
        {
            try
            {
                ...
                if (DGVEvents.Columns["EventName"] != null)
                {
                    ...
                    DGVEvents.Columns["EventName"].FillWeight = 25;
                }
                ...
                DGVEvents.ScrollBars = ScrollBars.Horizontal;
            ...
            {
                MessageBox.Show($"Error setting up columns: {ex.Message}",
                                "Column Setup Error",
        ...
        private void EventSave_Click(object sender, EventArgs e)
        {
            ...
            DateTime maxAllowedDate = now.AddDays(14);

            if (start <= now)
            {
                MessageBox.Show("please revisit Start date and time.",
                                "Invalid Schedule", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (start > maxAllowedDate)
            {
                MessageBox.Show("that's too far from the current Date.",
                                "Invalid Future Date", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (end <= start)
            {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (!isUpdateMode)
            {
                ...
                {
                    MessageBox.Show("Please select a schedule to edit first.", "Info",
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                isUpdateMode = true;
                ...
                try
                {
                    ...
                    WHERE ScheduleID = @ScheduleID";
                        ...
                        using (SqlCommand cmd = new SqlCommand(query, conn))
                        {
                            ...
                            if (reader.Read())
                            {
                                txtTruckCompany.Text = reader["TruckCompany"].ToString();
                                cmbDay.SelectedItem = reader["CollectionDay"].ToString();
                                cmbStatus.SelectedItem = reader["Status"].ToString();

                                if (TimeSpan.TryParse(reader["CollectionTime"].ToString(), out TimeSpan time))
                                    DTPGarbageSchedTime.Value = DateTime.Today.Add(time);
                            }
                        }
                    }

                    MessageBox.Show("Edit mode enabled. Modify the fields, then click 'Update' again to save changes.",
            ...
            {
                MessageBox.Show("Error updating garbage schedule: " + ex.Message,
                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        private bool ValidateGarbageInputs()
        {
            ...
            TimeSpan selectedTime = DTPGarbageSchedTime.Value.TimeOfDay;
            TimeSpan startAllowed = new TimeSpan(6, 0, 0); 
            TimeSpan endAllowed = new TimeSpan(18, 0, 0);    

            if (selectedTime < startAllowed || selectedTime > endAllowed)
            {
                MessageBox.Show("collection time between 6:00 AM to 6:00 PM.",
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                MessageBox.Show("Garbage schedule added successfully!",
                    "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
